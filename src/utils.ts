...
const setup =
  (JSON.parse(process.env.LOCAL_ACCOUNT || "") as Setup) || localSetup;
...
export function parseDID(did: string) {
  // This assumes the DID format is `did:algo:<network>:<appID>:<publicKey>`
  const matches = did.match(/^did:algo:([^:]+):([^:]+):([^:]+)$/);
  if (!matches) {
    return null;
  }
  return {
